...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    public void drawTree(Graphics2D g, Node n, int currentLevel, int x, int y) {
        if(n == null) {
            return;
        }
        if(n.left != null) {
            drawTree(g, n.left, currentLevel + 1, middle - (x+1) * xOff * (maxLevel - currentLevel), y + 1);
        }
        if(n.right != null) {
            drawTree(g, n.right, currentLevel + 1, middle + (x+1) * xOff * (maxLevel - currentLevel), y + 1);
        }
        g.setColor(n.color ? Color.red : Color.gray);
